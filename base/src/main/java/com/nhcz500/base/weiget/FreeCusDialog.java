...
public abstract class FreeCusDialog extends DialogFragment implements View.OnClickListener {
    ...
    private int elevation;//0不带阴影 其他则带阴影
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        dialog.getWindow().setBackgroundDrawableResource(R.color.transparent);//设置背景透明
        dialog.getWindow().getDecorView().setBackgroundResource(R.color.transparent); //设置背景
    ...
    private void setDialogView(ViewGroup view, int pxElevation) {
        if(getLayoutId()!=0){
            ...
            dialog.getWindow().setLayout(params.width>0?params.width+pxElevation*2:params.width,params.height>0?params.height+pxElevation*2:params.height);
